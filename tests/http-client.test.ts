...
import { IHttpRequestHandler } from "../src/http-request-handler";
import { IHttpResponse } from "../src/http-response";
import { HttpClientBuilder } from "../src";
...
describe("http-client with fake request handler", () => {
    ...
    it("should not be possible to add the same request/response converter twice", function () {
        ...
        client
            .addRequestConverter(requestConverter)
    ...
    it("should convert a HTTP request in reverse order", function () {
        ...
        client.addRequestConverter(first).addRequestConverter(second);
